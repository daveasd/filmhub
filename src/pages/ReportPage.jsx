import React, { useState } from 'react';
import { AlertTriangle, Loader2, CheckCircle2 } from 'lucide-react';
import StaticPageLayout, { StaticCard } from '../components/StaticPageLayout';
import { useToast } from '../contexts/ToastContext';
import { submitReport } from '../services/siteFormsService';

const CATEGORIES = [
  'Bug',
  'Broken movie data',
  'Broken image',
  'Login/account issue',
  'Review abuse',
  'AI recommendation issue',
  'Other',
];

export default function ReportPage({ user }) {
  const { toast } = useToast();
  const [category, setCategory] = useState(CATEGORIES[0]);
  const [message, setMessage] = useState('');
  const [email, setEmail] = useState(user?.email ?? '');
  const [loading, setLoading] = useState(false);
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!message.trim()) {
      toast('Please describe the problem.', 'info');
      return;
    }

    setLoading(true);
    try {
      const result = await submitReport({
        category,
        message,
        email,
        userId: user?.isGuest ? null : user?.id,
        username: user?.username ?? 'Guest',
      });
      setSubmitted(true);
      setMessage('');
      toast(
        result.storage === 'supabase'
          ? 'Report submitted. Thank you!'
          : 'Report saved locally. Thank you!',
      );
    } catch {
      toast('Could not submit report. Please try again.', 'info');
    } finally {
      setLoading(false);
    }
  };

  return (
    <StaticPageLayout
      title="Report a Problem"
      subtitle="Help improve FilmHub by telling us what went wrong."
      icon={AlertTriangle}
    >
      {submitted ? (
        <StaticCard className="text-center py-10">
          <CheckCircle2 className="h-12 w-12 text-brand-gold mx-auto mb-4" />
          <h2 className="text-xl font-bold text-white">Thank you</h2>
          <p className="text-gray-400 text-sm mt-2 max-w-md mx-auto">
            Your report was received. If you included contact details, I may follow up when
            possible.
          </p>
          <button
            type="button"
            onClick={() => setSubmitted(false)}
            className="mt-6 text-sm text-brand-gold font-semibold hover:underline"
          >
            Submit another report
          </button>
        </StaticCard>
      ) : (
        <StaticCard>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="report-category" className="block text-sm font-semibold text-gray-300 mb-2">
                Issue type
              </label>
              <select
                id="report-category"
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="w-full rounded-lg bg-dark-bg border border-dark-border text-white px-4 py-2.5 focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold"
              >
                {CATEGORIES.map((c) => (
                  <option key={c} value={c}>
                    {c}
                  </option>
                ))}
              </select>
            </div>

            <div>
              <label htmlFor="report-message" className="block text-sm font-semibold text-gray-300 mb-2">
                Description <span className="text-brand-red">*</span>
              </label>
              <textarea
                id="report-message"
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                rows={5}
                required
                placeholder="What happened? Include steps to reproduce if possible."
                className="w-full rounded-lg bg-dark-bg border border-dark-border text-white placeholder-gray-600 px-4 py-3 focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold resize-y"
              />
            </div>

            <div>
              <label htmlFor="report-email" className="block text-sm font-semibold text-gray-300 mb-2">
                Email (optional)
              </label>
              <input
                id="report-email"
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="you@example.com"
                className="w-full rounded-lg bg-dark-bg border border-dark-border text-white placeholder-gray-600 px-4 py-2.5 focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold"
              />
            </div>

            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg bg-brand-gold text-black font-bold px-6 py-3 hover:opacity-90 transition-opacity disabled:opacity-50 min-h-[44px]"
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : null}
              Submit report
            </button>
          </form>
        </StaticCard>
      )}
    </StaticPageLayout>
  );
}
